...
export async function upsertThread(thread: Omit<ChatThread, "created_at">): Promise<void> {
  ...
  // Try insert first — if already exists, just update updated_at (preserve title)
  ...
  if (error) {
    // Thread exists — only update updated_at, preserve title
...
export async function updateThreadTitle(threadId: string, userId: string, title: string): Promise<void> {
  await supabase.from("chat_threads")
