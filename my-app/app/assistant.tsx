...
import { ASHURA_MODEL } from "@/lib/model-config";
...
export const MODELS = [
  { id: ASHURA_MODEL, label: "ASHURA Evil V1", variant: "v1" as const },
  { id: ASHURA_MODEL, label: "ASHURA V2", variant: "v2" as const },
];

export type AshuraVariant = (typeof MODELS)[number]["variant"];
  ...
  return (
    <AssistantRuntimeProvider runtime={runtime}>
      <div className="flex min-h-0 flex-1 flex-col">
        {chatError && (
          <ChatErrorBanner message={chatError} onDismiss={() => setChatError(null)} />
        )}
...
function ChatErrorBanner({
  ...
  return (
    <div
      className="mx-3 mt-2 flex items-start gap-2 rounded-lg border border-red-800/50 bg-red-950/40 px-3 py-2 text-xs text-red-300 md:mx-4"
      role="alert"
    >
...
export const Assistant = () => {
  ...
  const [selectedVariant, setSelectedVariant] = useState<AshuraVariant>(MODELS[0].variant);
  const selectedModelConfig =
    MODELS.find((m) => m.variant === selectedVariant) ?? MODELS[0];
  ...
  useEffect(() => {
    if (!activeThreadId) {
      setActiveThreadId(window.crypto.randomUUID?.() || Math.random().toString(36).substring(7));
    }
  }, [activeThreadId]);
  ...
  const switchThread = useCallback(async (threadId: string) => {
    ...
    try {
      ...
      const history: InitMsg[] = (data.messages ?? []).map((m: { role: string; content: string; id?: string }) => ({
        id: m.id ?? (window.crypto.randomUUID?.() || Math.random().toString(36).substring(7)),
  ...
  const newChat = useCallback(() => {
    setInitialMessages([]);
    setActiveThreadId(window.crypto.randomUUID?.() || Math.random().toString(36).substring(7));  // fresh key → fresh runtime
    setSidebarOpen(false);
  }, []);
  ...
  const ModelSelector = (
    <div className="relative inline-flex items-center gap-1.5">
      ...
      <select
        ...
        onChange={(e) => setSelectedVariant(e.target.value as AshuraVariant)}
      ...
      >
        {MODELS.map((m) => (
          <option key={m.variant} value={m.variant} className="bg-black text-red-400">
            {m.label}
          </option>
        ))}
      </select>
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col">
        ...
        {loadingHistory ? (
          ...
          <ThreadContainer
            ...
            variant={selectedModelConfig.variant}
